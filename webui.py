...
with gr.Blocks(title="UVR5 WebUI") as app:
    gr.Markdown(
        value=
            "本软件以MIT协议开源, 作者不对软件具备任何控制力, 使用软件者、传播软件导出的声音者自负全责." + "<br>" + "如不认可该条款, 则不能使用或引用软件包内任何代码和文件. 详见根目录LICENSE."
    )
    with gr.Group():
        gr.Markdown(html_center("伴奏人声分离&去混响&去回声"),'h2')
        with gr.Group():
                gr.Markdown(
                    value=html_left("人声伴奏分离批量处理， 使用UVR5模型。" + "<br>" + \
                        ...
                        "3、个人推荐的最干净的配置是先MDX-Net再DeEcho-Aggressive。",'h4')
                )
                with gr.Row():
                    with gr.Column():
                    ...
                    with gr.Column():
                        ...
                        with gr.Column():
                            ...
                            with gr.Row():
                                vc_output4 = gr.Textbox(label="输出信息",lines=3)
                    but2.click(
                        uvr,
                        [
                            model_choose,
